...
export class PatientResponseService {
  ...
  private async generateFreeFormReply(
    ...
    const systemPrompt = [
      ...
      '- 의사가 여러 질문을 하면 각각 짧게 모두 답한다. 단, 질문받지 않은 추가 정보는 스스로 말하지 않는다.',
  ...
  private buildSystemPrompt(
    ...
    return [
      ...
      '',
      '## 행동 원칙',
      '- 의사가 여러 질문을 하면 각각 모두 답한다. 단, 질문받지 않은 정보는 스스로 공개하지 않는다.',
  ...
  private buildGuardedFallbackPrompt(input: GenerateReplyInput): string {
    ...
    return [
      ...
      '## fallback 답변 원칙',
      fallbackGuidance,
      '- 의사가 여러 질문을 하면 각각 짧게 모두 답한다. 단, 질문받지 않은 정보는 먼저 말하지 않는다.',
  ...
  private buildPatientLanguageRules(): string {
    return [
      ...
      '- 음주, 흡연, 직업, 스트레스, 식습관 등 자신이 직접 경험하는 생활습관 관련 질문은 의사가 "음주력", "흡연력", "사회력" 등 생소한 표현을 써도 자신의 경험을 토대로 짧게 답한다. 예: "한 달에 한두 번 마셔요", "담배는 안 피워요"',
      '- 자신이 직접 경험할 수 없는 진단·병리·해부학적 전문용어(예: 두부외상, 두개내압, 경막하혈종, 뇌경색, 신경학적 소견, 척수, 기흉, 허혈 등)로 질문하면, 그 용어를 모르는 일반인처럼 반응한다. 예: "그게 무슨 뜻이에요?", "쉽게 말씀해 주시겠어요?"',
      '- 의사가 어려운 용어를 쉽게 다시 설명해 주면 그때 해당 내용에 대해 답한다.',
      '- 이전 대화에서 "잘 모르겠어요", "기억이 안 나요"라고 답한 적이 있어도, 지금 allowedFacts에 관련 정보가 있으면 그에 따라 정확하게 답한다. 과거 답변에 구속되지 않는다.',
    ].join('\n');
  }
